...
1. Импортирован gorm
2. В модель =Location= добавлена модель из gorm - это нужно для взаимодействия с базой данных
3. При начале работы выполняется подключение к базе данных в файле =track.db=, выполняется миграция. Соединение кладем в переменную =*db*=.
4. После поступления координата сохраняются в табличке базы данных
